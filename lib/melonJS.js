...
(function( global, factory ){
	...
	if( typeof window === "undefined" && typeof module === "object" && typeof module.exports === "object" )
		...
		global.me = factory( global );
})(window, function( window, isServer ) {
	...
	(function(){
		...
		/**
		 * a deep copy function (using shorthand var from above variables)
		 * @ignore
		 */
	...
	me.game = (function() {
		...
		api.loadTMXLevel = function(level) {
			...
			// add all defined visible layers
			var array = api.currentLevel.getLayers(), i = array.length;
			while( i-- ){
				var layer = array[i];
				if( layer.visible )
			...
			var targetContainer = world;

			// load all ObjectGroup and Object definition
			array = api.currentLevel.getObjectGroups();
			i = array.length;
			while( i-- ){
				var group = array[i];
				...
				if( api.mergeGroup === false ){
					...
					world.addChild(targetContainer);
	...
	{		
		 ...
		 * @public
		 * @type Number
		 * @name me.AnimationSheet#defaultAnimationSpeed
		 */
		 ...
		 */
		//@TODO fix
		addAnimation : function(name, index, speed) {
			var anim = {
				name : name,
				frame : [],
				idx : 0,
				length : 0,
				speed: speed || this.defaultAnimationSpeed,
				nextFrame : 0
			};
			...
			for ( var i = 0 , len = index.length ; i < len; i++) {
				if (typeof(index[i]) === "number") {
					anim.frame[i] = this.textureAtlas[index[i]];
				} else { // string
					if (this.atlasIndices === null) {
						throw "melonjs: string parameters for addAnimation are only allowed for TextureAtlas ";
					} else {
						anim.frame[i] = this.textureAtlas[this.atlasIndices[index[i]]];
					}
				}
			}
			anim.length = anim.frame.length;
			this.anim[name] = anim;
		},
		 ...
		 *    return false; // do not reset to first frame
		 * }).bind(this));
		 * @TODO add ability to set from predefined object
		 **/
		setCurrentAnimation : function(name, resetAnim) {
			if (this.anim[name]) {
				...
				this.current.nextFrame = me.timer.getTime() + this.current.speed;
			} else {
				...
				for ( var i = this.children.length, obj; i--, obj = this.children[i];) {
					...
					if ( obj.isRenderable ) {
						...
						if (isTranslated) {
							x = obj.pos.x;
							y = obj.pos.y;
							globalTranslation.translate(obj.pos);
	...
	/** @scope me.ObjectEntity.prototype */ {
		...
		init : function(x, y, settings) {
			 ...
			 * @memberOf me.ObjectEntity
			 */
			this.gravity = me.sys.gravity || 0.98;
		...
		getBounds : function() {
			if (this.renderable) {
				...
				return this.renderable.getBounds().translate(this.pos);
			}
			return null;
		},
	...
	me.state = (function() {
		...
		// whether the game state is "paused"; false stops updates on (most) entities
		var _isPaused = false;
		...
		function _runLoop() {
			_activeUpdateFrame();
			if (_animFrameId !== -1){
				_animFrameId = window.requestAnimationFrame(_runLoop);
			}
		}
		...
		function _switchState(state) {
			...
			if (_screenObject[_state]) {
				if (_screenObject[_state].screen.addAsObject) {
		 ...
		 */
		api.set = function(state, screenObject) {
			_screenObject[state] = {};
			_screenObject[state].screen = screenObject;
			_screenObject[state].transition = true; //@TODO add default
		};
		 ...
		 */
		api.restart = function( resumeMusic ){
		 ...
		 * @param {Boolean} resumeTrack resume current track on screen resume
		 */
		api.resume = function(resumeMusic){
		 ...
		 * @param {Number} [duration=1000] expressed in milliseconds
		 */
		api.setGlobalTransition = function(effect, color, duration) {
			if (effect === "fade") {
		...
		api.change = function(state) {
			...
			_extraArgs = arguments.length > 1 ? Array.prototype.slice.call(arguments, 1) : null;

			// if fading effect
			if (_transition.duration && _screenObject[state].transition) {
				/** @ignore */
				_onSwitchComplete = function() {
					me.game.viewport.fadeOut(_transition.color, _transition.duration);
				};
				me.game.viewport.fadeIn(_transition.color, _transition.duration,
										function() {
		...
		api.loadLevel = function(levelId) {
			...
			if (levels[levelId] instanceof me.TMXTileMap) {
				...
				if( levels[api.getCurrentLevelId()] )
					levels[api.getCurrentLevelId()].reset();
